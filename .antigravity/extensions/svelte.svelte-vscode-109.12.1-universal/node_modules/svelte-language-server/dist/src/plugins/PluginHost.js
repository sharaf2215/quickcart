...
class PluginHost {
    ...
    rangeContains(parent, child) {
        return (this.comparePosition(parent.start, child.start) <= 0 &&
            this.comparePosition(child.end, parent.end) <= 0);
    }
    async getHierarchicalDocumentSymbols(textDocument, cancellationToken) {
        const flat = await this.getDocumentSymbols(textDocument, cancellationToken);
        const symbols = flat
            .map((s) => vscode_languageserver_1.DocumentSymbol.create(s.name, undefined, s.kind, s.location.range, s.location.range, []))
            .sort((a, b) => {
